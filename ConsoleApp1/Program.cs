...
List<Debtor> gmail_debtor = debtors.FindAll((debtor) => (debtor.Email.EndsWith("rhyta.com") || debtor.Email.EndsWith("dayrep.com")));
//foreach (var item in gmail_debtor) Console.WriteLine(item.ToString());
//-----------------------------------------------------------------------------


// Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag 
List<Debtor> yas_26_36 = debtors.FindAll((debtor)=> (DateTime.Now.Year - debtor.BirthDay.Year >= 26 && DateTime.Now.Year - debtor.BirthDay.Year <=36));
//foreach (var item in yas_26_36) Console.WriteLine(item.ToString());
//-----------------------------------------------------------------------------


// Borcu 5000 - den cox olmayan borclularic cixartmag
List<Debtor> borc_5000 = debtors.FindAll((debtor) => debtor.Debt < 5000);
//foreach (var item in borc_5000) Console.WriteLine(item.ToString());
//------------------------------------------------------------------------------


// Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
List<Debtor> simvol_18 = debtors.FindAll(debtor => debtor.FullName.Length > 18  && debtor.Phone.Count(c => c == '7') >= 2);
//foreach (var item in simvol_18) Console.WriteLine(item.ToString());
//-------------------------------------------------------------------------------


//Qishda anadan olan borclulari cixardmaq 
List<Debtor> qisda_dogulan = debtors.FindAll((debtor) => (debtor.BirthDay.Month == 12 || debtor.BirthDay.Month == 2 || debtor.BirthDay.Month == 2));
//foreach (var item in qisda_dogulan) Console.WriteLine(item.ToString());
//------------------------------------------------------------------------------
...
double ortalama_borc = debtors.Average((debtor) => debtor.Debt);
var borc_soyad = debtors.FindAll((debtor) => debtor.Debt > ortalama_borc).OrderBy((debtor) => debtor.FullName).ThenByDescending((debtor)=> debtor.Debt);
//foreach (var item in borc_soyad) Console.WriteLine(item.ToString());
//------------------------------------------------------------------------------
 

//Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq
List<Debtor> telefen_8_olmayan = debtors.FindAll((debtor) => debtor.Phone.Count(a => a == '8') == 0);
//foreach (var item in telefen_8_olmayan) Console.WriteLine(item.FullName+" --- " + (DateTime.Now.Year - item.BirthDay.Year) + " --- " + item.Debt);
//-----------------------------------------------------------------------------


//borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

var en_cox_dogulan_il = debtors.GroupBy(debtor => debtor.BirthDay.Year).OrderByDescending(deb=> deb.Count()).First().Key;
//Console.WriteLine(en_cox_dogulan_il);
//---------------------------------------------------------------------------


//Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
var borc_5_cox = debtors.OrderByDescending(debtor => debtor.Debt).Take(5);
//foreach (var item in borc_5_cox) Console.WriteLine(item.ToString());
//------------------------------------------------------------------------------

//Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
double cem = debtors.Sum(debtor => debtor.Debt);
//-------------------------------------------------------------------------------

//2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

List<Debtor> muharibe = debtors.FindAll((debtor) => (debtor.BirthDay >= new DateTime(1939,8,1) && debtor.BirthDay<= new DateTime(1945,8,2)));
//foreach (var item in muharibe) Console.WriteLine(item.ToString());
//-------------------------------------------------------------------------------


//Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
static bool tekrar(string phone)
{
    return phone.GroupBy(digit => digit).Any(group => group.Count() > 1);
}
var tekrarlanmayan = debtors.Where(debtor => !tekrar(debtor.Phone));
//foreach (var item in tekrarlanmayan) Console.WriteLine(item.ToString());
//-------------------------------------------------------------------------------
...
List<Debtor> ad_gunune_qeder = debtors.FindAll((debtor) => (debtor.Debt - 6000) <= 0);
//foreach (var item in ad_gunune_qeder) Console.WriteLine(item.ToString());
//----------------------------------------------------------------------------------


//Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
static bool s_m_i_l_e(string fullname, string smile)
{
    return smile.All(harf => fullname.Contains(harf));
}

var smile_adlar = debtors.FindAll((debtor) => s_m_i_l_e((debtor.FullName, "smile")));
